...
def calculate($grid):
    ...
    def sides($y; $x):
        # we only want to count side ends
        # we choose the plot that is at the end of rotating the direction 90 degrees clockwise
        # e.g., for the top side, we count it at the right-most plot sharing that side
        def has_fence($y1; $x1; $y2; $x2):
            # is there a fence on the $y2, $x2 side of $y1, $x1
            (in_bounds($y2; $x2) | not) or $grid[$y2][$x2] != $grid[$y1][$x1];
        ...
        def handle($dy; $dx):
            ...
            | ($y - $dx) as $pny
            | ($x + $dy) as $pnx
            # indirect fence square
            | ($y + $dy - $dx) as $idy
            | ($x + $dx + $dy) as $idx
            | if has_fence($y; $x; $dfy; $dfx)
                and (
                    (in_bounds($pny; $pnx) | not)
                    or ($grid[$y][$x] != $grid[$pny][$pnx])
                    or (has_fence($pny; $pnx; $idy; $idx) | not))
            then
                . + 1
            end;
        0 | handle(-1; 0) | handle(1; 0) | handle(0; -1) | handle(0; 1);
    ...
    | until (.[0] == $height;
        ...
        | until (.[1] == $width;
            ...
            | .[-1] | get_parent($key)
            | .[1] as $root
            | ($total + sides($y; $x) * ($size | getpath($root))) as $total
            | .[0]
            | [$y, $x + 1, $total, .])
